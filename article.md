...
де $S$ — потужність мел-спектрограми, $S_{ref} = 1.0$ (абсолютна шкала). Значення обрізаються до діапазону [–80, 0] дБ та нормалізуються до [0, 1].
...
де $d$ — симульована відстань, $d_{ref} = 10$ м. Додатково моделюється поглинання повітрям (зниження верхніх частот) та зменшення відношення сигнал/шум.
