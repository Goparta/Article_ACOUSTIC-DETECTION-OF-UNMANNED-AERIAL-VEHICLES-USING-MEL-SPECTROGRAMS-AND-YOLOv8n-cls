...
Запропонована система виконує бінарну класифікацію аудіосегментів ("дрон" / "не дрон") за таким конвеєром:

1. **Захоплення аудіо**: мікрофон записує звук із частотою дискретизації 16 кГц, моно.
2. **Сегментація**: аудіопотік розділяється на 1-секундні вікна (16 000 семплів).
3. **Мел-спектрограма**: кожен сегмент перетворюється на зображення 640×640 пікселів.
4. **Класифікація**: YOLOv8n-cls обробляє спектрограму та повертає ймовірності двох класів.
5. **Темпоральне згладжування**: мажоритарне голосування на основі 3 з 5 послідовних сегментів.
...
S_dB = 10 · log₁₀(S / S_ref)                    (1)

де S — потужність мел-спектрограми, S_ref = 1.0 (абсолютна шкала). Значення обрізаються до діапазону [–80, 0] дБ та нормалізуються до [0, 1].
...
Конвеєр обробки даних (рис. 1) включає наступні етапи:
1. Збір та організація: 24 502 вихідних файлів (7 657 дронів + 16 842 не-дронів).
2. Очищення: видалення тиші, кліпінгу, пошкоджених файлів — залишилось 21 646 (–11.5%).
3. Сегментація: нарізка на 1-секундні вікна — 62 007 сегментів.
4. Балансування: забезпечення співвідношення 1:1 між класами.
5. Аугментація: 19 типів, 5-кратне збільшення тренувальних даних — 227 950 зображень.
6. Валідаційний набір: 5 098 сегментів з повністю невідомих джерел.

![Рис. 1 — Data Processing Pipeline (Model v8)](figures/fig9_dataset_pipeline.png)
...
A(d) = –20 · log₁₀(d / d_ref)                    (2)

де d — симульована відстань, d_ref = 10 м. Додатково моделюється поглинання повітрям (зниження верхніх частот) та зменшення відношення сигнал/шум.
...
- Оптимізатор: AdamW;
- Learning rate: 0.001 (v8: постійний після warmup; v9: cosine annealing);
- Dropout: 0.3;
- Label smoothing: 0.05;
- Апаратне забезпечення: NVIDIA RTX 3090 (24 ГБ VRAM).

Модель v8 тренувалась 32 епохи (~9.3 години), модель v9 — 44 епохи з cosine annealing. Криві навчання (рис. 2) демонструють характерну різницю

![Рис. 2 — Training Curves for Models v8 and v9](figures/fig2_training_curves.png)
...
Найбільш значущі переходи:
- **v1→v2**: виявлення data leakage — сегментний поділ даних давав 98.9% на валідації, але лише 3–11% на реальних записах. Вирішення: source-level split.
- **v6→v7**: додавання hard negatives з ESC-50 знизило false positive rate на мові з 48% до 0%.
- **v7→v8**: два ключових покращення: (а) зміна ref=np.max на ref=1.0 — одна строчка коду, +15% до drone recall; (б) збільшення роздільності до 640×640 — helicopter FP знизився з 3.3% до 0%.
...
Обидві продуктивні моделі (v8 та v9) оцінювались на валідаційних наборах, повністю ізольованих від тренувальних даних на рівні джерел записів. Загальні метрики наведено на рис. 3.

![Рис. 3 — Classification Performance on Unseen Validation Data](figures/fig1_overall_results.png)
...
Детальний аналіз помилок класифікації представлено у вигляді confusion matrices (рис. 4).

![Рис. 4 — Confusion Matrices for Models v8 and v9](figures/fig7_confusion_matrices.png)
...
Здатність виявляти дрони на відстані є критичною для практичного застосування. Обидві моделі тестувались на валідаційних даних з аугментацією відстані від 20 до 300 м (рис. 5).

![Рис. 5 — Drone Detection Recall vs. Distance](figures/fig3_distance_detection.png)
...
Для оцінки робастності кожна модель тестувалась двічі: на чистих валідаційних даних та на тих самих даних з аугментацією (шум, вітер, дощ, стиснення, зміна pitch тощо), що імітує реальні умови розгортання (рис. 6).

![Рис. 6 — Robustness: Clean vs. Augmented Validation Conditions](figures/fig8_robustness.png)
...
Критичною особливістю цього тесту є те, що обидві моделі обробляють **однакові дані**, що забезпечує коректне порівняння. Результати по категоріях наведено на рис. 7.

![Рис. 7 — YouTube Test: Accuracy by Sound Category](figures/fig4_youtube_per_category.png)
...
Детальний аналіз по кожному файлу (рис. 8) показує, що на чистому звуці FPV-дрона обидві моделі досягають 92%, тоді як на відео з порівнянням DJI — лише 17%/24%, оскільки ≈2/3 відео складають коментарі автора.

![Рис. 8 — YouTube Test: Per-File Results](figures/fig5_youtube_per_file.png)

### 4.5. Аналіз помилок

Аналіз хибних спрацьовувань на не-дронових YouTube-файлах (рис. 9) виявляє основні проблемні категорії:

![Рис. 9 — YouTube Test: False Positives on Non-Drone Audio](figures/fig6_youtube_false_positives.png)

- **Малий пропелерний літак**: v8 — 44.4% FP, v9 — 27.8%. Пропелерний звук акустично подібний до дрона у частотному діапазоні 200–2000 Гц.
- **Мотоцикл Triumph Street Triple**: v8 — 25.1%, v9 — 14.0%. Високочастотний вихлоп створює патерн, схожий на електромотор дрона.
- **Гелікоптер Dolby Atmos**: v8 — 0%, v9 — 20.7%. Високоякісний запис з вираженими гармоніками частково нагадує дрон.

При цьому низка складних категорій розпізнається безпомилково: бензопила Stihl — v8: 0%, v9: 0.6%; Harley Iron 883 — обидві 0%; Cessna 172 — v8: 0%, v9: 1.4%.
...
Ключові методологічні висновки, отримані за 9 ітерацій розробки:

1. **Source-level split** є обов'язковим для аудіокласифікаторів — сегментний поділ призводить до витоку даних та критично завищених метрик (98.9% → 3% реальна точність).

2. **Абсолютна шкала дБ** (ref=1.0) зберігає інформацію про гучність та відстань — одна строчка коду дала +15% до drone recall та уможливила детекцію на відстані.

3. **Роздільність спектрограми** (640×640 vs 224×224) забезпечує у 12.7 разів більше інформації, що дозволило знизити helicopter FP з 3.3% до 0%.

4. **Hard negatives** (ESC-50) є критичними для зменшення хибних спрацьовувань на побутових звуках.
...
- багатокласова класифікація (ідентифікація типу БПЛА);
- інтеграція мікрофонного масиву для визначення напрямку;
- зменшення false positives для акустично схожих джерел (літаки, мотоцикли);
- адаптивне навчання моделі при розгортанні в нових акустичних середовищах.
